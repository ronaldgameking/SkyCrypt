    ...
    for(const item of items){
        ...
        if(itemLore.length > 0){
            ...
            if(item.extra?.spawned_for){
                ...
                const spawnedForUser = await helper.resolveUsernameOrUuid(spawnedFor, db, { cacheOnly: cacheOnly });
...
module.exports = {
    ...
    getStats: async (db, profile, allProfiles, items, cacheOnly = false) => {
        ...
        const playerObject = await helper.resolveUsernameOrUuid(profile.uuid, db, {cacheOnly: cacheOnly});
        ...
        .all(
            Object.keys(profile.members).map(a => helper.resolveUsernameOrUuid(a, db, {cacheOnly: cacheOnly}))
        );
    ...
    getProfile: async (db, paramPlayer, paramProfile, options = { cacheOnly: false }) => {
        if(paramPlayer.length != 32){
            try{
                const { uuid } = await helper.resolveUsernameOrUuid(paramPlayer, db, options);
